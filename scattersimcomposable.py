...
def random_test_composable_delme(
        A,
        system_params,
        initial_state_list,
        ci_seq_list,
...
def random_test_composable(
    ...
    for abs_state in initial_state_list:
        ...
        if ic is not None:
            ...
            x_samples = SaMpLe.sample_ival_constraints(ic, A.plant_abs.num_samples)
...
#         pi_array = None
    ...
    while sim_num < A.N:
        ...
        if A.num_dims.pi == 0:
            ...
            pi_lb_list = [np.tile(pi_cons.l, (A.plant_abs.num_samples, 1))
                          for pi_cons in pi_cons_list]
            pi_ub_list = [np.tile(pi_cons.h, (A.plant_abs.num_samples, 1))
                          for pi_cons in pi_cons_list]
...
#             for (idx, xf) in enumerate(rchd_concrete_state_array.iterable()):
#                 if xf.x in system_params.final_cons:
#                     res.append(idx) #TODO trace_list
#                     print(x0_array[idx, :], d0_array[idx, :], '->', '\t', xf.x, xf.d)
#                     #if A.num_dims.ci != 0:
#                     #    print('ci:', ci_array[idx])
#                     #if A.num_dims.pi != 0:
