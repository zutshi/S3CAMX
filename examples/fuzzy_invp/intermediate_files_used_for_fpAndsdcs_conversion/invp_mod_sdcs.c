/*
 * The same as invp_mod.c, but now uses ZOH and is part of a SDCS: sampled data
 * control system.
 * This means that the controller is no longer called inside deriv() and is
 * instead called in the main.
 */
...
int main(void) {
  ...
  double
    u,
    t,
    h = 0.01,             /* Integration step size. 0.1 does not work, and < 0.01 seems to be fine*/
    //y[3] = {0.1,0.0,0.0},  /* Initial conditions.     */
    y[3],
    tmax = 100;              /* Simulation time period. */

  double yl[3] = {-.4, -0.3, -0.0},
         yh[3] = {0.4, 0.3, 0.0};
  ...
  while(num_sims < MAX_SIMS)
  {
    ...
    while(t<tmax) {
      ...
      e_edot_[0] = (int)((-y[0])*CONVERSION_FACTOR);
      e_edot_[1] = (int)((-y[1])*CONVERSION_FACTOR);
      iv.x_arr = e_edot_;
      rv.output_arr = &u_;
      controller(&iv, &rv);
