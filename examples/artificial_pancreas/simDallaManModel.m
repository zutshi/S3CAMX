...
% Call ODE45 solver
[T,x] = ode45(@dallaManODE, [0,simTime], startState, options);		%0:1:simTime  %ERROR THROWN: The last entry in tspan must be different from the first entry.
% Final state
finState=x(end,:);
...
times = T + startTime * ones(size(T));
    ...
    function [y] = dallaManODE(t,x)
        ...
        %%D = mCarbs;
        insulinInput=getInsulinInput(t);
        Ra=getMealInput(t);
