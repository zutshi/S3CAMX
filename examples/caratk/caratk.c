...
{
    if(n<=PERIOD/2) return n/10.;
    else return (PERIOD-n)/10.;
}
    ...
    //double x_hat_m1_1, double x_hat_m1_2, double u_m1, double ref_1, double ref_2, double y_1, double y_2, double attk_1, double attk_2, double w_1, double w_2)
{
	...
	*/
        ...
        double x_hat_m1_1, x_hat_m1_2, u_m1, ref_1, ref_2, y_1, y_2;
        double attk_1, attk_2, w_1, w_2;
        //clk
        unsigned int n;
	//Threshold triggering level
	const double threshold = 20;
	//Kalman gain matrix parameters
	const double k11 = 0.6180; const double k12 = 0.0011;
	const double k21 = 0.0011; const double k22 = 0.6180;
	...
	double y_a_1, y_a_2, res_1, res_2, x_hat_1, x_hat_2, u;

        y_1 = input->x_arr[0];
        y_2 = input->x_arr[1];

        x_hat_m1_1 = input->float_state_arr[0];
        x_hat_m1_2 = input->float_state_arr[1];
        u_m1 = input->float_state_arr[2];
        ...
        attk_1 = input->input_arr[0];
        attk_2 = input->input_arr[1];
        w_1 = input->input_arr[2];
        w_2 = input->input_arr[3];

	//ProcessorOutputs GatherOuts;

	//Compute sesor values received by the estimator-control block
	y_a_1 = y_1 + attk_1 + w_1;
	y_a_2 = y_2 + attk_2 + w_2;
	//Compute residue of the estimator
	res_1 = y_a_1 - ((a11*x_hat_m1_1 + a12*x_hat_m1_2) + (b11*u_m1));
	res_2 = y_a_2 - ((a21*x_hat_m1_1 + a22*x_hat_m1_2) + (b21*u_m1));
	//Compute new estimated states
	x_hat_1 = (a11*x_hat_m1_1 + a12*x_hat_m1_2) + (b11*u_m1) + (k11*res_1 + k12*res_2);
	x_hat_2 = (a21*x_hat_m1_1 + a22*x_hat_m1_2) + (b21*u_m1) + (k21*res_1 + k22*res_2);
	//Compute controller output
	u = l11*(x_hat_1 - ref_1) + l12*(x_hat_2 - ref_2);

	ret_val->output_arr[0] = u;

	//ret_val->output_arr[1] = ((res_1*res_1 + res_2*res_2) > threshold*threshold);

        if(myabs(res_1) + myabs(res_2)>threshold)
            ret_val->output_arr[1] = 1;
        else
            ret_val->output_arr[1] = 0;
        //ret_val->output_arr[1] = (myabs(res_1) + myabs(res_2))>threshold;



        //printf("%f\n", ret_val->output_arr[1]);
	ret_val->output_arr[2] = /*sqrt(res_1*res_1 + res_2*res_2)*/ myabs(res_1) + myabs(res_2);

	ret_val->float_state_arr[0] = x_hat_1;
	ret_val->float_state_arr[1] = x_hat_2;
	ret_val->float_state_arr[2] = u;

        if(n>PERIOD) ret_val->int_state_arr[0] = 0;
        else
        ret_val->int_state_arr[0] = n + 1;
